...
def decide_network(args, train_env, device):
    from mappo.algorithms.ramppo_network import Network, Q_Net, Dueling_Net
    ...
    if network_type == None:
        raise Exception("Check a hyperparamete: network!")
...
def decide_target(Replay_buffer, behavior_network, target_network, args, device):
    ...
    train_targets: Dict[str, object] = {
        "dqn": Target_Dqn,
        "double_dqn": Target_Double_Dqn,
    }
    target_type: Callable[[str], object] = train_targets.get(args.decide_target, None)
    if target_type == None:
        raise Exception("Check a hyperparameter: decide_target!")
    train_module = target_type(
        Replay_buffer, behavior_network, target_network, args, device
    )
...
def cal_td_error(action, reward, done, behavior_q, target_q, gamma):
    ...
    behavior_value = (
        torch.gather(input=behavior_q[0], dim=1, index=action_index)
        .reshape(1, -1)[0]
        .sum(dim=0)
    )
