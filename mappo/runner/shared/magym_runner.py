...
class MAGYM_Runner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            while not all(dones):
                ...
                next_obs, rewards, dones, infos = self.train_env.step(actions[0]) # 일단은 batch_size를 1인 상태에서만 돌아가도록 설정해놓았음

                share_obs = self.obs_sharing(obs = next_obs, n_agents = self.num_agents)
                rewards = self.convert_rewards(rewards)

                data = next_obs, share_obs, rewards, np.array([dones]), infos, values, actions, action_log_probs, rnn_states, rnn_states_critic 
            ...
            if episode % self.log_interval == 0:
                ...
                print(f"Algo {self.algorithm_name} Exp {self.experiment_name} updates {episode}/{episodes} episodes, Accumulated Rewards {accumulated_rewards} \n")
    ...
    def insert(self, data):
        ...
        self.buffer.insert(share_obs, obs, rnn_states, rnn_states_critic,
                           actions, action_log_probs, values, rewards, masks) , #active_mask
        ...
        for test_number in range(self.eval_episodes):
            ...
            while not all(eval_dones):
                eval_actions, eval_rnn_states = \
                    self.trainer.policy.act(obs = np.concatenate([eval_obs]),
                ...
                eval_next_obs, eval_rewards, eval_dones, eval_infos = self.eval_env.step(eval_actions[0])
