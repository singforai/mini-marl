...
class ReplayBuffer:
    ...
    def sample_chunk(self, batch_size, chunk_size):
        ...
        return (
            torch.tensor(s_lst, dtype=torch.float).view(
                batch_size, chunk_size, n_agents, obs_size
            ),
            torch.tensor(a_lst, dtype=torch.float).view(
                batch_size, chunk_size, n_agents
            ),
            torch.tensor(r_lst, dtype=torch.float).view(
                batch_size, chunk_size, n_agents
            ),
            torch.tensor(s_prime_lst, dtype=torch.float).view(
                batch_size, chunk_size, n_agents, obs_size
            ),
...
class QNet(nn.Module):
    def __init__(self, observation_space, action_space, recurrent=False):
        ...
        for agent_i in range(self.num_agents):
            ...
            if recurrent:
                setattr(
                    self,
                    "agent_gru_{}".format(agent_i),
                    nn.GRUCell(self.hx_size, self.hx_size),
                )
            setattr(
                self,
                "agent_q_{}".format(agent_i),
    ...
    def forward(self, obs, hidden):
        ...
        for agent_i in range(self.num_agents):
            ...
            if self.recurrent:
                x = getattr(self, "agent_gru_{}".format(agent_i))(
                    x, hidden[:, agent_i, :]
                )
                next_hidden[agent_i] = x.unsqueeze(1)
            q_values[agent_i] = getattr(self, "agent_q_{}".format(agent_i))(
                x
            ).unsqueeze(1)
...
def train(
    ...
    for _ in range(update_iter):
        ...
        mix_net_hidden = [
            torch.empty_like(mix_net_target_hidden) for _ in range(_chunk_size + 1)
        ]
        ...
        for step_i in range(_chunk_size):
            ...
            pred_q, next_mix_net_hidden = mix_net(
                q_a, s[:, step_i, :, :], mix_net_hidden[step_i]
            )

            max_q_prime, target_hidden = q_target(
                s_prime[:, step_i, :, :], target_hidden.detach()
            )
            max_q_prime = max_q_prime.max(dim=2)[0].squeeze(-1)
            q_prime_total, mix_net_target_hidden = mix_net_target(
                max_q_prime, s_prime[:, step_i, :, :], mix_net_target_hidden.detach()
            )
            target_q = r[:, step_i, :].sum(dim=1, keepdims=True) + (
                gamma * q_prime_total * (1 - done[:, step_i])
            )
            ...
            target_hidden[done_mask] = q_target.init_hidden(
                len(target_hidden[done_mask])
            )
            mix_net_hidden[step_i + 1][~done_mask] = next_mix_net_hidden[~done_mask]
            mix_net_hidden[step_i + 1][done_mask] = mix_net.init_hidden(
                len(mix_net_hidden[step_i][done_mask])
            )
            mix_net_target_hidden[done_mask] = mix_net_target.init_hidden(
                len(mix_net_target_hidden[done_mask])
            )
        ...
        torch.nn.utils.clip_grad_norm_(
            mix_net.parameters(), grad_clip_norm, norm_type=2
        )
        optimizer.step()
...
def test(env, num_episodes, q):
    ...
    for episode_i in range(num_episodes):
        ...
        with torch.no_grad():
            ...
            while not all(done):
                action, hidden = q.sample_action(
                    torch.Tensor(state).unsqueeze(0), hidden, epsilon=0
                )
                next_state, reward, done, info = env.step(
                    action[0].data.cpu().numpy().tolist()
                )
                score += sum(reward)
                state = next_state
...
def main(
    ...
    optimizer = optim.Adam(
        [{"params": q.parameters()}, {"params": mix_net.parameters()}], lr=lr
    )
    ...
    for episode_i in range(max_episodes):
        epsilon = max(
            min_epsilon,
            max_epsilon
            - (max_epsilon - min_epsilon) * (episode_i / (0.6 * max_episodes)),
        )
        state = env.reset()
        done = [False for _ in range(env.n_agents)]
        with torch.no_grad():
            ...
            while not all(done):
                action, hidden = q.sample_action(
                    torch.Tensor(state).unsqueeze(0), hidden, epsilon
                )
        ...
        if episode_i % log_interval == 0 and episode_i != 0:
            ...
            print(
                "#{:<10}/{} episodes , avg train score : {:.1f}, test score: {:.1f} n_buffer : {}, eps : {:.1f}".format(
                    episode_i,
                    max_episodes,
                    train_score,
                    test_score,
