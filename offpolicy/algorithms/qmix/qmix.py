...
from algorithms.vdn.algorithm.vdn_mixer import VDNMixer
from algorithms.base.trainer import Trainer
...
class QMix(Trainer):
    def __init__(self, args, num_agents, batch_size, policies, policy_mapping_fn, device=torch.device("cuda:0"), episode_length=None, vdn=False):
        ...
        self.policy_agents = {policy_id: sorted([agent_id for agent_id in range(self.num_agents) if self.policy_mapping_fn(agent_id) == policy_id]) for policy_id in self.policies.keys()}
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None):
        ...
        for p_id in self.policy_ids: #policy_0 요소 하나밖에 없음
            ...
            if policy.multidiscrete:
                ...
                """ 
                pol_q_seq = policy.q_values_from_actions(pol_all_q_seq[:-1], stacked_act_batch) # pol_q_seq : 60x96x1

            agent_q_out_sequence = pol_q_seq.split(split_size=batch_size, dim=-2) # 60x96x1 => (60x32x1, 60x32x1, 60x32x1)

            agent_q_seq.append(torch.cat(agent_q_out_sequence, dim=-1))  # (60x32x1, 60x32x1, 60x32x1) =>(60, 32, 3)
            ...
            target_q_seq = target_q_seq[1:] # [60, 96, 1]
        ...
        agent_q_seq = torch.cat(agent_q_seq, dim=-1) # (60, 32, 3)    만약 agent 별 다른 policy를 가진다면 append가 효과를 발휘해서 stack되겠지만 어림도 없지
        agent_nq_seq = torch.cat(agent_nq_seq, dim=-1) # (60, 32, 3)

        # get curr step and next step Q_tot values using mixer
        Q_tot_seq = torch.reshape(self.mixer(agent_q_seq, cent_obs_batch[:-1]),(-1,self.batch_size,1)) # ((60, 32, 3) , (60, 32, 48)) => [60, 32, 1]
        next_step_Q_tot_seq = torch.reshape(self.target_mixer(agent_nq_seq, cent_obs_batch[1:]),(-1,self.batch_size,1))

        # agents share reward
        rewards = to_torch(rew_batch[self.policy_ids[0]][0]).to(**self.tpdv) # 3x60x32x1 =>60x32x1([0]번째 인덱스 이므로 ) 

        # form bad transition mask
        bad_transitions_mask = torch.cat((torch.zeros(1, batch_size, 1).to(**self.tpdv), dones_env_batch[:self.episode_length - 1, :, :])) #self.episode_length는 60임 => 첫 차원인 (1 +59, 32, 1)
        ...
        error = (Q_tot_seq - Q_tot_target_seq.detach()) * (1 - bad_transitions_mask) #60 32 1
        if self.use_per:
        ...
        loss.backward()
        grad_norm = torch.nn.utils.clip_grad_norm_(self.parameters, self.args.max_grad_norm) #gradient 행렬의 크기가 max_grad_norm을 넘어가면 gradient에 (max_grad_norm/gradient의 크기)를 곱해준다. 
        self.optimizer.step()
        # log
        train_info = {}
