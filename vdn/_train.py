...
class Train_Target:
    ...
    def sample_chunk(self, batch_size, chunk_size):
        states, actions, rewards, next_states, dones, priority_idx, is_weight = self.replay_buffer.sample(batch_size, chunk_size)
    ...
    def initialize_hidden(self, batch_size1, batch_size2, batch_size3 = None):
        hidden  = self.behavior_network.init_hidden(batch_size = batch_size1).to(self.device)
        target_hidden = self.target_network.init_hidden(batch_size = batch_size2).to(self.device)
        if batch_size3 == None:
            return hidden, target_hidden
        else:
            double_hidden = self.target_network.init_hidden(batch_size = batch_size3).to(self.device)
    ...
    def train(self, target_network, optimizer, epsilon):
        ...
        for _ in range(self.update_iter):
            ...
            for step in range(self.chunk_size):
                state, action, reward, next_state, done = super().extract_step(step, states, actions, rewards, next_states, dones)
                sum_max_behavior_q, next_hidden = super().sum_behavior_value(state = state.to(self.device), 
                                                                             hidden = hidden, 
                                                                             action = action)

                target_q, next_target_hidden = target_network(obs = next_state.to(self.device),hidden = target_hidden.detach())
                max_target_q = target_q.max(dim=2)[0]
                sum_max_target_q = max_target_q.sum(dim=1, keepdims=True)
                target_values = is_weight * (reward + self.gamma * (1 - done) * sum_max_target_q).sum(dim=1, keepdims=True)
            ...
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(parameters=self.behavior_network.parameters(), max_norm=self.grad_clip_norm, norm_type=2)
            optimizer.step()
        ...
        for _ in range(self.update_iter):
            ...
            for step in range(self.chunk_size):
                ...
                sum_max_behavior_q, next_hidden = super().sum_behavior_value(state = state.to(self.device), 
                                                                             hidden = hidden, 
                                                                             action = action)
                target_q, next_target_hidden = target_network(next_state.to(self.device), target_hidden.detach())
                ...
                target_values = is_weight * (reward + self.gamma * (1 - done) * sum_double_q).sum(dim=1, keepdims=True)
                loss += F.mse_loss(target_values.detach(), sum_max_behavior_q)
                done_mask = done.squeeze(-1).bool()
...
#         iter_loss: float = 0
#         for _ in range(self.update_iter):
#             states, actions, rewards, next_states, dones, priority_idx, importance_sampling_weight = Replay_buffer.sample(
#                 self.batch_size, self.chunk_size)
#             hidden = behavior_network.init_hidden(
#                 batch_size=self.batch_size).to(self.device)
#             target_hidden = target_network.init_hidden(
#                 batch_size=self.batch_size).to(self.device)
#             loss = 0.0
...
#                 behavior_q, next_hidden = behavior_network(
#                     state.to(self.device), hidden)
#                 max_behavior_q = behavior_q.gather(
#                     dim=2, index=action).squeeze(dim=-1)
...
#                 done_mask = done.squeeze(-1).bool()
#                 next_hidden[done_mask] = behavior_network.init_hidden(
#                     batch_size=len(next_hidden[done_mask])).to(self.device)
#                 next_target_hidden[done_mask] = behavior_network.init_hidden(
#                     batch_size=len(next_target_hidden[done_mask])).to(self.device)
...
#             optimizer.zero_grad()
#             loss.backward()
#             torch.nn.utils.clip_grad_norm_(parameters=behavior_network.parameters(
...
#                 next_target_hidden[done_mask] = behavior_network.init_hidden(
#                     batch_size=len(next_target_hidden[done_mask])).to(self.device)
#                 next_double_hidden[done_mask] = behavior_network.init_hidden(
#                     batch_size=len(next_double_hidden[done_mask])).to(self.device)
