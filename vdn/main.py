...
def main(args):
    ...
    # wandb.init(project='minimal-marl', entity='singfor7012',
    #            name=args.experiment_name, reinit=True) if use_wandb else None
    ...
    target_network, behavior_network = action_network(args, train_env)
    ...
    target_module = target_setting(args = args)

    test = Test(test_env, args)
        ...
        with torch.no_grad():
            ...
            while not all(done):
                action, next_hidden, behavior_q = behavior_network.sample_action(
                    obs=torch.Tensor(state).unsqueeze(dim=0), hidden=hidden, epsilon=epsilon)
                
                next_state, reward, done, info = train_env.step(action[0])

                target_q, next_target_hidden  = target_network(torch.tensor(next_state).unsqueeze(dim=0), target_hidden)
    ...
    for episode in tqdm(range(max_episodes), desc="training", ncols=70):
        ...
        with torch.no_grad():
            ...
            target_hidden = target_network.init_hidden()
            while not all(done):
                action, next_hidden, behavior_q = behavior_network.sample_action(obs=torch.tensor(state).unsqueeze(dim=0), hidden=hidden, epsilon=epsilon)
