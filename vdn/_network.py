...
class Network(nn.Module):
    ...
    def action_network(self, agent_i: int = 0):
        self.action_net.append(nn.Sequential(nn.Linear(self.gru_hidden, self.action_hidden),
                                             nn.Linear(self.action_hidden, self.action_space[agent_i].n)))
...
class Dueling_Net(Network):
    ...
    def forward(self, obs, hidden):
        ...
        next_hidden: list = ([torch.empty(obs.shape[0], 1, self.gru_hidden)] * self.num_agents)
        ...
        for agent_i in range(self.num_agents):
            ...
            advantage_values[agent_i] = self.advantage_net[agent_i](feature).unsqueeze(dim=1)
            value_values[agent_i] = self.value_net[agent_i](feature).unsqueeze(dim=0)

            action_values[agent_i] = value_values[agent_i] + (advantage_values[agent_i] - advantage_values[agent_i].mean(dim=2))
...
#         self.action_hidden_size: int = 16
...
#             setattr(self, f'action_network_{agent_i}', nn.Sequential(nn.Linear(self.gru_hidden_size, self.action_hidden_size),
#                                                                      nn.Linear(self.action_hidden_size, action_space[agent_i].n)))
...
#             setattr(self, f'advantage_network_{agent_i}', nn.Sequential(nn.Linear(self.gru_hidden_size, self.action_hidden_size),
#                                                                         nn.Linear(self.action_hidden_size, action_space[agent_i].n)))
#             setattr(self, f'value_network_{agent_i}', nn.Sequential(nn.Linear(self.gru_hidden_size, self.action_hidden_size),
#                                                                     nn.Linear(self.action_hidden_size, 1)))
...
#             advantage_values[agent_i] = getattr(
#                 self, f'advantage_network_{agent_i}')(feature_output).unsqueeze(dim=1)
#             value_values[agent_i] = getattr(self, f'value_network_{agent_i}')(
#                 feature_output).unsqueeze(dim=0)
