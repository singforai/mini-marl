...
def action_network(args, train_env, device):
    if args.action_network == "q_net":
        ...
        target_mix_net = Mix_Net(
            observation_space=train_env.observation_space, args=args
        ).to(device)
        behavior_q_net = Q_Net(
            observation_space=train_env.observation_space,
            action_space=train_env.action_space,
            args=args,
        ).to(device)
        behavior_mix_net = Mix_Net(
            observation_space=train_env.observation_space, args=args
        ).to(device)
